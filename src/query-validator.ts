...
const ALLOWED_PATTERNS_WITH_FORBIDDEN_KEYWORDS = [
  /^SHOW\s+CREATE\s+/i,           // SHOW CREATE TABLE, SHOW CREATE DATABASE, etc.
  /^SHOW\s+GRANTS\s*/i,           // SHOW GRANTS - displays grant info
...
export function validate(query: string): ValidationResult {
  ...
  if (!ALLOWED_STATEMENTS.includes(firstKeyword as AllowedStatement)) {
    return {
      valid: false,
      error: `Query rejected: Only SELECT, SHOW, DESCRIBE, and EXPLAIN statements are allowed. Found: ${firstKeyword || 'unknown'}`
    };
  }
